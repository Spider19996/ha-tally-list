...
async def _log_logging_toggle(hass, user_id, option: str, enabled: bool) -> None:
    ...
    await hass.async_add_executor_job(
        _write_price_list_log, hass, name, action, option
    )


def _get_flow_user_id(hass, context) -> str | None:
    context_id = context.get("user_id") if context and context.get("user_id") else None
...
class TallyListConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_logging(self, user_input=None):
        if user_input is not None:
            ...
            new_log_pin_set = user_input[CONF_LOG_PIN_SET]
            if new_log_pin_set != self._log_pin_set:
                await _log_logging_toggle(
